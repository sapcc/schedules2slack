...
build-all: build/schedules2slack

build/schedules2slack: FORCE
	go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/schedules2slack ./cmd
...
install: FORCE build/schedules2slack
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/schedules2slack "$(DESTDIR)$(PREFIX)/bin/schedules2slack"

# which packages to test with "go test"
GO_TESTPKGS := $(shell go list -f '{{if or .TestGoFiles .XTestGoFiles}}{{.ImportPath}}{{end}}' ./...)
# which packages to measure coverage for
GO_COVERPKGS := $(shell go list ./... | grep -E '/internal' | grep -Ev '/test')
...
help: FORCE
	...
	@printf "  \e[36mbuild/schedules2slack\e[0m   Build schedules2slack.\n"
...
docker-build:
	docker buildx build --platform linux/arm64 . -t ${IMAGE}/schedules2slack:${VERSION}
	docker buildx build --platform linux/arm64 . -t ${IMAGE}/schedules2slack:latest
docker-push:
